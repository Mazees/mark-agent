...
<p align="center">
  <a href="https://www.npmjs.com/package/@mazees/mark"><img src="https://img.shields.io/npm/v/@mazees/mark?style=for-the-badge&color=1fb854&logo=npm" alt="npm version"></a>
  <a href="https://github.com/Mazees/mark-agent"><img src="https://img.shields.io/badge/Platform-Windows_10%2F11-blue?style=for-the-badge&logo=windows" alt="Platform"></a>
  <a href="https://nodejs.org/"><img src="https://img.shields.io/badge/Node.js-%3E%3D20-339933?style=for-the-badge&logo=node.js" alt="Node.js"></a>
  <a href="https://github.com/Mazees/mark-agent/releases/"><img src="https://img.shields.io/badge/Download-Releases-blue?style=for-the-badge&logo=github" alt="Download"></a>
</p>
...
- **Arsitektur Decoupled (Node.js Core + Modern WebUI):** Inti aplikasi dipisahkan menjadi server Node.js performa tinggi (`src/server/`) dengan WebSocket Hub (`/stream`) dan antarmuka React 19 / Vite 7 (`src/renderer/`). Berjalan ringan dalam mode Microsoft Edge App Mode terisolasi (dengan fallback otomatis ke browser default) dan CLI Live Activity Monitor (`mark`).
- **Autonomous Multi-Agent Sub-Agent Engine (Mission Control):** Mark bertindak sebagai _Lead Agent_ yang mampu memecah tugas kompleks dan mendelegasikannya ke banyak **Sub-Agent spesialis** yang bekerja secara paralel di latar belakang (`spawn_subagent`, `wait_subagents`). Dilengkapi dengan antarmuka **Mission Control** dan **Live Sub-Agent Intercom HUD** yang menampilkan pemikiran mendalam (_Reasoning Analisis_), langkah eksekusi berkala (_Execution Steps_), dan laporan hasil akhir dengan dukungan penuh Markdown & Syntax Highlighting.
- **Operator-Style Execution Timeline & Parameter Reason:** Visualisasi langkah eksekusi ReAct bergaya modern operator dengan status badge lipat (_collapsible details_), parameter `reason` terstruktur pada setiap eksekusi tool, dan rendering intervensi pengguna langsung di alur langkah eksekusi (_in-flight user intervention_).
- **Interactive File Diff Modal & Syntax Safety:** Inspeksi visual perbedaan berkas (_file diff_) sebelum dan sesudah perubahan kode diterapkan. Mendukung peninjauan baris kode yang ditambah/dihapus secara interaktif serta validasi sintaksis otomatis (_syntax validator_) sebelum penulisan ke disk untuk mencegah kerusakan berkas kode.
- **Kapasitas Konteks Masif (256K Tokens) & Zero-Recalc Caching:** Manajemen konteks cerdas berkapasitas hingga 256.000 token dengan caching token persisten di basis data SQLite. Mencegah komputasi ulang token yang boros sumber daya dan memungkinkan dialog teknis serta analisis repositori kode bervolume besar secara instan.
...
- **Relational Growth System & Tactile Avatar:** Hubungan Anda dengan Mark dievaluasi layaknya manusia sungguhan melalui 4 parameter krusial (_Warmth, Sarcasm, Trust, Energy_) dengan preservasi riwayat mood AI dan respons interaksi visual taktil pada avatar. Tingkat kesopanan, kelancangan (_roasting_), dan kepribadian Mark akan berevolusi organik sesuai gaya komunikasi Anda.
- **Multi AI Provider (Built-in Gemini / DeepSeek / Local / Cloud):** Mark hadir dengan **Google Gemini Engine (Gratis)** sebagai _provider_ bawaan yang siap pakai tanpa API Key. Anda juga memiliki fleksibilitas penuh untuk menggunakan **DeepSeek Web RPC**, **Local AI** (LM Studio), **Cloud AI** (Groq / Cerebras), maupun _Custom OpenAI-Compatible API_.
...
- **Git Workspace Tools (`git-*`):** `git-status`, `git-diff`, `git-commit`, `git-revert`.
- **Google Workspace Suite (`google-*` / `gdrive-*` / `gcal-*` / `gmail-*`):** `gdrive-search`, `gdrive-list`, `gdrive-read`, `gdrive-upload`, `gdrive-create`, `gdrive-move`, `gdrive-copy`, `gdrive-share`, `gdrive-info`, `gcal-list`, `gcal-create`, `gcal-delete`, `gmail-search`, `gmail-read`, `gmail-send`, `gmail-mark-read`.
...
- **Telegram Bot Integration:** Sinkronisasi notifikasi, pengiriman tangkapan layar, dan kontrol perintah jarak jauh melalui Telegram.

---

## Arsitektur Memory Persistence & Integritas Fakta

Arsitektur memori Mark dirancang untuk kontinuitas ingatan jangka panjang (_Long-Term Memory Persistence_) secara lokal tanpa bergantung pada cloud:
...
- **Node.js**: Versi 20 atau lebih baru ([Unduh Node.js LTS](https://nodejs.org/))
...
### Cara 1: Instalasi Instan via NPM (Direkomendasikan)
...
_MARK Core Server akan langsung aktif, membuka Live Activity Monitor di terminal, dan meluncurkan antarmuka WebUI dalam mode desktop terisolasi (Microsoft Edge App Mode)._
...
### Cara 2: Menjalankan dari Source Code (Development / Kontributor)

Jika Anda ingin memodifikasi kode sumber atau berkontribusi pada pengembangan MARK:
...
4. **Jalankan aplikasi (Production mode):**

   ```bash
   npm start
   ```

5. **Menjalankan UI & Server secara terpisah (Dev Mode Hot-Reload):**
...
6. **Mendaftarkan perintah `mark` lokal (Global Symlink):**
