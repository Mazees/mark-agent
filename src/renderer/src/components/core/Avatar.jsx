...
const Avatar = ({
  ...
  useEffect(() => {
    ...
    // Update accent colors on mood elements
    const moodColor = getMoodColor(mood, status).hex
  ...
  const updateAutonomousQuirks = useCallback((now) => {
    const s = stateRef.current
    if (s.isSeeingDesktop || s.status === 'speaking' || s.isSleeping || s.status === 'sleeping') {
      s.targetRotY = 0
      s.targetRotZ = 0
      s.targetSaccadeX = 0
      s.targetSaccadeY = 0
    ...
    if (now - s.lastQuirkTime >= s.nextQuirkInterval) {
      ...
      // 2. Subtle Listening Nod
      else if (roll < 0.55) {
        s.quirkPitchOffset = -9
        s.quirkRollOffset = 0
        setTimeout(() => {
          s.quirkPitchOffset = 0
        }, 350)
      }
      // 3. Floating Stretch Breath
      else if (roll < 0.75) {
        s.quirkAltitudeOffset = -12
  ...
  useEffect(() => {
    ...
    const updateFrame = (now) => {
      ...
      let addPitch = effectiveStatus === 'sleeping' ? 14 : effectiveStatus === 'listening' ? -6 : 0
      ...
      const totalY = harmonicY + s.pokeOffsetY + desktopDownwardY + s.quirkAltitudeOffset
      if (robotRootRef.current) {
        robotRootRef.current.style.transform = `translate3d(${harmonicX}px, ${totalY}px, ${desktopForwardZ}px) scale3d(${breathScaleX}, ${breathScaleY}, 1)`
      }
      ...
      const antTargetLeft =
        effectiveStatus === 'speaking'
          ? Math.sin(t * 6) * (s.intensity * 6)
          : -s.velRotY * 0.35 +
            (effectiveStatus === 'thinking' ? -15 : effectiveStatus === 'listening' ? 14 : 0)
      ...
      >
        ...
        >
          ...
          >
            ...
            <g className="avatar-core-beat origin-[65px_70px]">
              <circle cx="65" cy="70" r="9" fill="#1fb854" className="avatar-mood-fill" />
        ...
        >
          ...
          >
            ...
            <circle cx="20" cy="46" r="3.5" fill="#1fb854" className="avatar-mood-fill" />
            <circle cx="30" cy="46" r="3.5" fill="#1fb854" className="avatar-mood-fill" />
            <circle cx="8" cy="26" r="3" fill="#1fb854" className="avatar-mood-fill" />
          </svg>
        </div>
        ...
        >
          ...
          >
            ...
            <circle cx="20" cy="46" r="3.5" fill="#1fb854" className="avatar-mood-fill" />
            <circle cx="30" cy="46" r="3.5" fill="#1fb854" className="avatar-mood-fill" />
            <circle cx="42" cy="26" r="3" fill="#1fb854" className="avatar-mood-fill" />
          </svg>
        </div>
        ...
        <div
          ref={thrusterRef}
          className="absolute left-[118px] top-[305px] w-[84px] h-[22px] rounded-full bg-[#1fb854]/40 blur-[10px] pointer-events-none"
          style={{ transform: 'translateZ(-35px)' }}
        />
        <div
          ref={groundShadowRef}
          className="absolute left-[90px] top-[360px] w-[140px] h-[26px] rounded-full bg-black/75 blur-[8px] pointer-events-none transition-transform duration-75"
