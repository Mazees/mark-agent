      ...
      >
        ...
        {showMentionList && (mentionDisplayItems.length > 0 || isLoadingFiles) && (
          <div className="absolute bottom-full left-4 mb-2 w-[380px] max-w-[calc(100vw-2rem)] bg-base-300/95 backdrop-blur-xl border border-[var(--glass-border)] rounded-xl p-1.5 shadow-[0_8px_32px_rgba(0,0,0,0.5)] z-50 animate-fade-in flex flex-col">
            {/* Header: Hanya jika kategori terpilih atau saat loading */}
            {(mentionCategory !== 'all' || isLoadingFiles) && (
              <div className="px-2 py-1 mb-1 flex items-center justify-between text-xs border-b border-white/5 pb-1">
                {mentionCategory !== 'all' ? (
                  <button
                    type="button"
                    onClick={() => {
                    ...
                    className="flex items-center gap-1 text-primary hover:underline cursor-pointer text-xs font-medium"
                  >
                    <ChevronLeft size={13} />
                    <span>Semua (@)</span>
                  </button>
                ) : (
                  <span className="text-[11px] text-gray-400 font-medium">Mention</span>
                )}
                {isLoadingFiles && (
